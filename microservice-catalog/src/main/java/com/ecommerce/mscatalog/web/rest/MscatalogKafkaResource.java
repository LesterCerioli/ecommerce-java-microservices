...
@RestController
@RequestMapping("/api/mscatalog-kafka")
public class MscatalogKafkaResource {

    private final Logger log = LoggerFactory.getLogger(MscatalogKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public MscatalogKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
